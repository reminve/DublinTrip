    ...
    <div className="space-y-5">
      ...
      {userProfile?.is_admin && (
        <div 
          ...
          className="border border-dashed border-slate-300 dark:border-slate-800 hover:border-emerald-500/40 rounded-2xl p-6 text-center bg-slate-50 dark:bg-slate-900/10 hover:bg-emerald-500/[0.01] hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
        >
          <input 
            type="file" 
            id="react-file-input" 
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="w-10 h-10 bg-slate-100 dark:bg-slate-900/80 rounded-xl flex items-center justify-center border border-slate-200 dark:border-slate-850">
              <UploadCloud className="w-5 h-5 text-emerald-500 dark:text-emerald-400" />
            </div>
            <p className="text-xs font-bold text-slate-700 dark:text-slate-350">Ajouter des photos du voyage</p>
                ...
                <div className="absolute bottom-2 left-3 right-3 flex items-center justify-between text-[10px] text-white font-bold drop-shadow">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`flex items-center gap-0.5 hover:text-rose-400 transition-colors ${isLiked ? 'text-rose-400' : 'text-slate-200'}`}
                    >
            ...
            <div className="w-full md:w-85 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-900 p-4 flex flex-col justify-between space-y-4 overflow-y-auto bg-white dark:bg-slate-950">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-900 pb-3">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{lightboxPhoto.date}</span>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                      (lightboxPhoto.likes || []).includes(currentUser)
                        ...
                        : 'bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-850 text-slate-600 dark:text-slate-400 hover:text-rose-500'
                    }`}
                  >
                ...
                <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
                  ...
                  {(lightboxPhoto.comments || []).length === 0 ? (
                    ...
                    (lightboxPhoto.comments || []).map(comment => (
                      <div key={comment.id} className="bg-slate-50 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-850 rounded-xl p-3 space-y-1">
